...
compared pipelines on a pair of RNA-seq samples. With necessary  meta
...
number of pipeline compared), with each column representing a sample and each
...
In order to fit into function 'matrixFilter', necessary transformation to
...
scales as Cufflinks pipelines, as we selected `unitReference` as signals from
...
However, two types of non-expressed features should be analyzed separately:

### Features expressed in one technical replicate but not the other.

The reverse accumulated proportions of such either-or expressed features are
plotted stratified by the detrended signals as described previously.
...
A strategy that summarizes the overlapped proportions among top differential
...
Or, $y/0=inf$ with $y>0$ can be considered into fold changes by comparing
$y$ only.
```{r,eval=FALSE}
